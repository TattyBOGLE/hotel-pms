...
export default function HousekeepingPage() {
  const [tasks, setTasks] = useState<Housekeeping[]>([]);
  ...
  const loadTasks = async () => {
    try {
      const response = await housekeepingApi.getAll();
  ...
  const handleStatusUpdate = async (id: string, status: Housekeeping['status']) => {
    try {
        ...
        <div className="sm:flex sm:items-center">
          <div className="sm:flex-auto">
            ...
            <p className="mt-2 text-sm text-gray-700">
              Manage room cleaning tasks and track their status.
            </p>
          </div>
                    ...
                    {tasks.map((task) => (
                      <tr key={task.id}>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <select
                            value={task.status}
                            onChange={(e) => handleStatusUpdate(task.id, e.target.value as Housekeeping['status'])}
